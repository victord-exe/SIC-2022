...
def upload_appointment(appointment):
    ...
    df.loc[len(df)] = ["cita:  " + appointment]
...
def start_telegram():
    ...
    def cmd_cita(message):
        markup = ForceReply()
        msg = bot.send_message(message.chat.id, "Su cita: esta asignada para: 23/3/2023", reply_markup=markup)
    ...
    def datos_cu(message):
        ...
        if message.text == "Sacar Cita":
            ...
            msg = bot.send_message(message.chat.id, "Por favor selecione un recuadro", reply_markup=markup)
            bot.register_next_step_handler(msg, menu)
...
def run_discord():
   # buttonPage.destroy()
    stemmer = LancasterStemmer('spanish')
    tag = ''
